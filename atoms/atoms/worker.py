...
# Max age for turnstile tokens in the queue (seconds).
# Cloudflare turnstile tokens are typically valid ~300s.
# We discard tokens older than this to avoid wasting API calls.
TOKEN_MAX_AGE_S = 270
...
def _rotate_proxy_for_request(
    session: c_requests.Session,
    *,
    worker_kind: str,
    worker_id: int,
    args: argparse.Namespace,
    proxy_pool: Optional[RotatingProxyPool],
    enabled: bool = True,
    current_proxy: str = "",
    reason: str = "",
) -> str:
    """Rotate to a new proxy for the next request. Returns the new proxy URL.

    Unlike _apply_proxy_for_worker_session which is called once at worker start,
    this is called before each critical request to get a fresh proxy, especially
    after rate-limit (4429) or temporary errors (4001).
    """
    if not bool(args.proxy_enabled) or not bool(enabled) or not proxy_pool:
    ...
    if new_proxy != current_proxy:
        ...
        _safe_print(
            f"[{worker_kind}-{worker_id}] rotated proxy -> {mask_proxy_url(new_proxy)}{reason_str}"
        )
    return new_proxy
...
def _disable_proxy_for_session(
    session: c_requests.Session,
    *,
    worker_kind: str,
    worker_id: int,
    ...
    session.proxies = {}
    _safe_print(f"[{worker_kind}-{worker_id}] proxy disabled -> direct reason={reason}")
...
def _emit_task_result(task_id: int, payload: Dict[str, Any]) -> None:
    out = {
        ...
        "file_key_sync_status": str(payload.get("file_key_sync_status") or ""),
        "file_key_sync_error": str(payload.get("file_key_sync_error") or ""),
...
def _file_key_sync_headers(bearer_token: str) -> Dict[str, str]:
    headers = {
...
def _file_key_sync_query(args: argparse.Namespace) -> Dict[str, Any]:
    provider_name = str(args.file_key_sync_provider_name or "").strip()
    if provider_name:
        return {"name": provider_name}
    provider_index = int(args.file_key_sync_provider_index)
    if provider_index >= 0:
        return {"index": provider_index}
    raise RuntimeError("file-key sync requires provider name or non-negative provider index")


def _normalize_file_key_entries(raw_entries: Any) -> list[Dict[str, str]]:
    ...
    for item in raw_entries:
        if not isinstance(item, dict):
            continue
        api_key = str(item.get("api-key") or "").strip()
        if not api_key:
            continue
        entry: Dict[str, str] = {"api-key": api_key}
        proxy_url = str(item.get("proxy-url") or "").strip()
        if proxy_url:
...
def _is_missing_file_key_entries_error(exc: Exception) -> bool:
    msg = str(exc or "").strip().lower()
    if not msg:
        return False
    return any(
...
def _sync_app_ai_key_to_file_entries(app_ai_key: str, args: argparse.Namespace) -> str:
    key = str(app_ai_key or "").strip()
    if not key:
        raise RuntimeError("file-key sync requires non-empty app_ai_key")

    base_url = str(args.file_key_sync_base_url or "").strip().rstrip("/")
    if not base_url:
        raise RuntimeError("file-key sync base url is empty")

    endpoint = f"{base_url}/v0/management/openai-compatibility/file-key-entries"
    headers = _file_key_sync_headers(str(args.file_key_sync_bearer_token or ""))
    params = _file_key_sync_query(args)
    timeout_s = int(args.file_key_sync_timeout_s)

    s = c_requests.Session()
    entries: list[Dict[str, str]] = []
    try:
        resp = s.get(endpoint, headers=headers, params=params, timeout=timeout_s)
        data = _json_or_error("GET file-key-entries", resp)
        entries = _normalize_file_key_entries(data.get("api-key-entries"))
    except Exception as exc:
        if _is_missing_file_key_entries_error(exc):
            entries = []
        else:
            raise

    desired_proxy = str(args.file_key_sync_proxy_url or "").strip()
    for item in entries:
        if str(item.get("api-key") or "").strip() != key:
            continue
        current_proxy = str(item.get("proxy-url") or "").strip()
        if desired_proxy and current_proxy != desired_proxy:
            item["proxy-url"] = desired_proxy
        elif not desired_proxy and "proxy-url" in item:
            item.pop("proxy-url", None)
        payload = {"index": entries.index(item), "value": item}
        resp = s.patch(endpoint, headers=headers, params=params, json=payload, timeout=timeout_s)
        _json_or_error("PATCH file-key-entries", resp)
        return "updated"

    new_entry: Dict[str, str] = {"api-key": key}
    if desired_proxy:
        new_entry["proxy-url"] = desired_proxy
    entries.append(new_entry)
    resp = s.put(
        endpoint,
        headers=headers,
        params=params,
        json={"api-key-entries": entries},
        timeout=timeout_s,
    )
    _json_or_error("PUT file-key-entries", resp)
    return "added"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable HTTP proxy for register/apikey workers (default: disabled).",
    )
    parser.add_argument(
        "--no-proxy-enabled",
        dest="proxy_enabled",
    ...
    parser.add_argument(
        "--no-proxy-register-enabled",
        dest="proxy_register_enabled",
        action="store_false",
        help="Disable proxy on register/send-magic-link flow.",
    )
    parser.add_argument(
        "--proxy-apikey-enabled",
        action="store_true",
        default=False,
        help="Use proxy on apikey/chat flow (default: disabled).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--file-key-sync-enabled",
        action="store_true",
        default=True,
        help="Append extracted APP_AI_KEY to CLIProxyAPI file-key-entries endpoint (default: enabled).",
    )
    parser.add_argument(
        "--no-file-key-sync-enabled",
        dest="file_key_sync_enabled",
        action="store_false",
        help="Disable automatic APP_AI_KEY sync to file-key-entries endpoint.",
    )
    parser.add_argument(
        "--file-key-sync-base-url",
        default="http://localhost:8317",
        help="CLIProxyAPI base URL for file-key-entries management API.",
    )
    parser.add_argument(
        "--file-key-sync-bearer-token",
        default="dfg55",
        help="Bearer token for file-key-entries management API.",
    )
    parser.add_argument(
        "--file-key-sync-provider-name",
        default="bedrock",
        help="Provider name for file-key-entries API query. If empty, falls back to --file-key-sync-provider-index.",
    )
    parser.add_argument(
        "--file-key-sync-provider-index",
        type=int,
        default=-1,
        help="Provider index for file-key-entries API query when name is empty.",
    )
    parser.add_argument(
        "--file-key-sync-proxy-url",
        default="",
        help="Optional proxy-url field to store alongside the appended APP_AI_KEY.",
    )
    parser.add_argument(
        "--file-key-sync-timeout-s",
        type=int,
        default=15,
        help="HTTP timeout seconds for file-key-entries sync requests.",
    )
...
class RegisterRateLimiter:
    """Cross-worker rate limiter for send-magic-link calls.

    Atoms.dev rate-limits send-magic-link globally (not per-IP), so multiple
    workers firing simultaneously all get 4429.  This limiter ensures that:
    - At most one send-magic-link request is in-flight across all workers
    - After a 4429, ALL workers pause for an escalating cooldown
    - After a success, the minimum gap is shortened

    This turns the chaotic "all workers spam → all 4429" into an orderly
    serialized flow: worker-1 fires → waits → worker-2 fires → waits → ...
    """

    def __init__(
        self,
        min_gap_s: float = 5.0,
        rate_limit_base_s: float = 15.0,
        rate_limit_backoff: float = 1.5,
        rate_limit_max_s: float = 120.0,
        success_gap_s: float = 3.0,
    ):
        self._lock = threading.Lock()
        self._last_request_time: float = 0.0
        self._min_gap_s = float(min_gap_s)
        self._current_gap_s = float(min_gap_s)
        self._rate_limit_base_s = float(rate_limit_base_s)
        self._rate_limit_backoff = float(rate_limit_backoff)
        self._rate_limit_max_s = float(rate_limit_max_s)
        self._success_gap_s = float(success_gap_s)
        self._consecutive_429 = 0
        # Serialize: only one worker sends at a time
        self._send_lock = threading.Lock()

    def acquire(self) -> None:
        """Block until it's this worker's turn and the cooldown has elapsed."""
        self._send_lock.acquire()
        # Now we hold the exclusive send slot; wait for the gap
        with self._lock:
            gap = self._current_gap_s
    ...
    def release_success(self) -> None:
        """Call after a successful send-magic-link (code == 0)."""
        with self._lock:
            self._last_request_time = time.time()
            self._consecutive_429 = 0
            self._current_gap_s = self._success_gap_s
        self._send_lock.release()

    def release_rate_limited(self) -> float:
        """Call after a 4429 response. Returns the cooldown applied."""
        with self._lock:
            self._consecutive_429 += 1
            cooldown = self._rate_limit_base_s * (
                self._rate_limit_backoff ** min(self._consecutive_429 - 1, 6)
            )
            cooldown = min(cooldown, self._rate_limit_max_s)
            self._current_gap_s = cooldown
            self._last_request_time = time.time()
        self._send_lock.release()
        return cooldown

    def release_error(self, code: int = 0) -> None:
        """Call after a non-429 error (4001, 5007, etc.)."""
        with self._lock:
            self._last_request_time = time.time()
            # Small bump on error but don't escalate like 429
            self._current_gap_s = max(self._min_gap_s, self._current_gap_s)
        self._send_lock.release()
    ...
    def status(self) -> str:
        with self._lock:
            return (
                f"gap={self._current_gap_s:.1f}s "
...
def turnstile_worker(
    ...
    while not state.done_event.is_set():
        try:
            ...
            while not state.done_event.is_set():
                # Check if token has gone stale while waiting for queue space
                if time.time() - produced_at > TOKEN_MAX_AGE_S:
                    _safe_print(f"[Turnstile-{worker_id}] Token expired while waiting for queue, discarding")
                    break
                try:
                    token_queue.put((captcha, produced_at), timeout=1.0)
                    break
                except queue.Full:
                    continue
        except Exception as e:
...
def register_worker(
    ...
    proxy_pool: Optional[RotatingProxyPool] = None,
    rate_limiter: Optional[RegisterRateLimiter] = None,
):
    s = c_requests.Session()
    register_proxy_active = bool(args.proxy_register_enabled)
    current_proxy = _apply_proxy_for_worker_session(
        s,
        worker_kind="Register",
        worker_id=worker_id,
        args=args,
        enabled=register_proxy_active,
        proxy_pool=proxy_pool,
    )

    while not state.done_event.is_set():
        # --- Get a turnstile token with staleness check ---
        try:
            item = token_queue.get(timeout=1.0)
        except queue.Empty:
            continue

        # Unpack (token, timestamp) tuple
        if isinstance(item, tuple) and len(item) == 2:
            captcha, token_time = item
        else:
            captcha, token_time = item, time.time()  # legacy fallback

        tokens_taken = 1
        try:
            # Check token freshness before using it
            token_age = time.time() - token_time
            if token_age > TOKEN_MAX_AGE_S:
                _safe_print(
                    f"[Register-{worker_id}] Discarding stale token (age={token_age:.0f}s > {TOKEN_MAX_AGE_S}s)"
                )
                state.add_failure()
                continue
            ...
            _safe_print(f"[Register-{worker_id}] Using token for {email} (age={token_age:.0f}s)")
            ...
            for token_round in range(1, refresh_retries + 2):
                ...
                # Rotate proxy at the start of each token round
                if proxy_pool and register_proxy_active:
                    current_proxy = _rotate_proxy_for_request(
                        s,
                        worker_kind="Register",
                        worker_id=worker_id,
                        args=args,
                        ...
                        reason=f"token_round={token_round}",
                    )
                ...
                while True:
                    ...
                    # --- Acquire the global rate limiter before sending ---
                    # This serializes send-magic-link across ALL workers
                    if rate_limiter:
                        rate_limiter.acquire()

                    # Re-check token freshness after waiting for rate limiter
                    waited_age = time.time() - token_time
                    if waited_age > TOKEN_MAX_AGE_S:
                        if rate_limiter:
                            rate_limiter.release_error()
                        _safe_print(
                            f"[Register-{worker_id}] Token expired while waiting for rate limiter "
                            f"(age={waited_age:.0f}s)"
                        )
                        send_ok = False
                        break
                    ...
                    try:
                        ...
                        if rate_limiter:
                            rate_limiter.release_error()
                        if _is_proxy_transport_error(send_exc) and register_proxy_active:
                            if proxy_pool and current_proxy:
                                proxy_pool.mark_bad(current_proxy)
                            register_proxy_active = False
                            current_proxy = ""
                            _disable_proxy_for_session(
                                s,
                                worker_kind="Register",
                                worker_id=worker_id,
                                reason=f"transport-error:{send_exc}",
                            )
                            continue
                        raise send_exc
                    ...
                    if code == 4429:
                        if proxy_pool and current_proxy:
                            proxy_pool.mark_rate_limited(current_proxy)
                        if rate_limiter:
                            cooldown = rate_limiter.release_rate_limited()
                            _safe_print(
                                f"[Register-{worker_id}] 4429 -> global cooldown {cooldown:.1f}s "
                        ...
                        if proxy_pool and register_proxy_active:
                            current_proxy = _rotate_proxy_for_request(
                                ...
                                enabled=register_proxy_active,
                                current_proxy=current_proxy,
                                reason="4429-rate-limit",
                            )
                        if left_4429 > 0:
                            left_4429 -= 1
                            # Don't sleep here — the rate_limiter.acquire() at
                            # the top of the next iteration handles the wait
                            continue
                        break
                    ...
                    if code == 4001:
                        if rate_limiter:
                            rate_limiter.release_error(code)
                        if proxy_pool and current_proxy:
                            proxy_pool.mark_bad(current_proxy)
                            if register_proxy_active:
                                current_proxy = _rotate_proxy_for_request(
                                    s,
                                    worker_kind="Register",
                                    worker_id=worker_id,
                                    ...
                                    enabled=register_proxy_active,
                                    current_proxy=current_proxy,
                                    reason="4001-try-later",
                                )
                        if left_4001 > 0:
                            left_4001 -= 1
                            _safe_print(
                                f"[Register-{worker_id}] 4001 retry (left={left_4001})"
                            )
                            continue
                        break
                ...
                if code in (5007, 4001, 4429) and token_round < (refresh_retries + 1):
                    _safe_print(
                        f"[Register-{worker_id}] send-magic-link code={code}, need fresh token "
                        f"(token_round={token_round}/{refresh_retries + 1})"
                    )
                    try:
                        new_item = token_queue.get(timeout=15.0)
                        tokens_taken += 1
                        if isinstance(new_item, tuple) and len(new_item) == 2:
                            captcha, token_time = new_item
                        else:
                            captcha, token_time = new_item, time.time()
                        new_age = time.time() - token_time
                        if new_age > TOKEN_MAX_AGE_S:
                            _safe_print(
                                f"[Register-{worker_id}] Fresh token also stale (age={new_age:.0f}s), skipping"
                            )
                            break
                    except queue.Empty:
                        break
    ...
    while not state.done_event.is_set():
        ...
        try:
            ...
            for chat_attempt in range(1, max_chat_attempts + 1):
                ...
                # Rotate proxy between chat attempts for better distribution
                if chat_attempt > 1 and proxy_pool and bool(args.proxy_apikey_enabled):
                    current_proxy = _rotate_proxy_for_request(
                        s,
                        worker_kind="APIKey",
                        worker_id=worker_id,
                        ...
                        enabled=True,
                        current_proxy=current_proxy,
                        reason=f"chat-attempt-{chat_attempt}",
                    )
                ...
                try:
                    ...
                    if _api_code(sent) != 0:
                        code = _api_code(sent)
                        if code in (4001, 4429) and proxy_pool and current_proxy:
                            if code == 4429:
                                proxy_pool.mark_rate_limited(current_proxy)
                            else:
                                proxy_pool.mark_bad(current_proxy)
            ...
            file_key_sync_status = "disabled"
            file_key_sync_error = ""
            if bool(args.file_key_sync_enabled):
                try:
                    file_key_sync_status = _sync_app_ai_key_to_file_entries(app_ai_key, args)
                    _safe_print(
                        f"[APIKey-{worker_id}] file-key sync status={file_key_sync_status} "
                        f"provider={str(args.file_key_sync_provider_name or args.file_key_sync_provider_index)}"
                    )
                except Exception as sync_exc:
                    file_key_sync_status = "failed"
                    file_key_sync_error = str(sync_exc)
                    _safe_print(f"[APIKey-{worker_id}] file-key sync failed: {sync_exc}")
            ...
            jwt_info.update({
                ...
                "file_key_sync_status": file_key_sync_status,
                "file_key_sync_error": file_key_sync_error,
            })
...
def main() -> int:
    ...
    # Atoms.dev rate-limits send-magic-link globally (not per-IP).
    # The rate limiter serializes requests across all register workers
    # so they take turns instead of all firing simultaneously.
    register_rate_limiter = RegisterRateLimiter(
        min_gap_s=5.0,
        rate_limit_base_s=15.0,
    ...
    # Use workers count directly — the rate limiter handles serialization.
    # Multiple register workers are still useful because while one waits for
    # email verification (~30-60s), another can fire the next send-magic-link.
    reg_workers = max(2, workers) if count > 1 else workers
    api_workers = max(2, workers) if count > 1 else workers
    ...
    _safe_print(
        f"[Main] reg_workers={reg_workers} api_workers={api_workers} "
        f"rate_limiter=enabled(gap={register_rate_limiter._min_gap_s:.1f}s)"
    )
    register_threads: list[threading.Thread] = []
    for i in range(reg_workers):
        t = threading.Thread(
            target=register_worker,
            args=(i + 1, args, state, token_queue, jwt_queue, shared_proxy_pool, register_rate_limiter),
            daemon=True,
        )
        t.start()
        register_threads.append(t)

    # API Key Consumers
    apikey_threads: list[threading.Thread] = []
    for i in range(api_workers):
        ...
        t.start()
        apikey_threads.append(t)

    all_worker_threads = turnstile_threads + register_threads + apikey_threads
    pipeline_start = time.time()
    # Hard pipeline timeout: 2 hours max
    MAX_PIPELINE_S = 7200.0
    last_proxy_log = 0.0
    PROXY_LOG_INTERVAL = 60.0  # Log proxy health every 60s
    ...
    try:
        while not state.done_event.is_set():
            ...
            if elapsed > MAX_PIPELINE_S:
                _safe_print(f"[Main] Pipeline timeout ({MAX_PIPELINE_S:.0f}s) reached; stopping.")
                state.done_event.set()
                break
            ...
            if not any(t.is_alive() for t in all_worker_threads):
                _safe_print("[Main] All worker threads have exited; stopping.")
                state.done_event.set()
                break

            # Periodic status log
            if (now - last_proxy_log) >= PROXY_LOG_INTERVAL:
                parts = []
                if shared_proxy_pool:
                    parts.append(
                        f"proxy: {shared_proxy_pool.get_status_summary()} "
                        f"avail={shared_proxy_pool.available_count()}/{len(shared_proxy_pool.urls)}"
                    )
                parts.append(f"rate_limiter: {register_rate_limiter.status}")
                parts.append(
                    f"ok={state.success_count}/{state.target_count} fail={state.failure_count} "
                    f"elapsed={elapsed:.0f}s"
                )
                _safe_print(f"[Main] {' | '.join(parts)}")
                last_proxy_log = now
